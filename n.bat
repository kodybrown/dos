...
:parse
    ...
    if /i "%~1"=="hosts" (
        ...
        if exist "%bin%\elevate.cmd" (
            if exist "%bin%\Notepad2.exe" start "nircmd hosts" "%bin%\elevate.cmd" "%bin%\Notepad2.exe" "c:\Windows\System32\drivers\etc\hosts"
            if not exist "%bin%\Notepad2.exe" start "nircmd hosts" "%bin%\elevate.cmd" "Notepad.exe" "c:\Windows\System32\drivers\etc\hosts"
            endlocal && exit /B 0
        )
